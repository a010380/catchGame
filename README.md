...
3. **每日摘要** — 每天早上（06:30 之後）推一則「接下來 24 小時的賽事」。
4. **行事曆訂閱** — 另外產生一個 `.ics`，iPhone 訂閱後可以直接在行事曆上瀏覽賽程
   （預設**不響鈴**，因為提醒已經交給 Telegram，兩邊都開會收到兩次通知）。

目前啟用 **10 個聯賽**：MLB、NBA、NFL、英超、西甲、CPBL、LCK、LCP、MSI、Worlds。

賽季外的聯賽抓到 0 場是正常的（例如 NBA 在 7–10 月休賽、Worlds 的賽程要等 Riot 公布）。
不想看某個聯賽就在 `config.py` 把它的 `enabled` 改成 `False`。
...
| 檔案 | 作用 |
| --- | --- |
| `config.py` | **唯一需要你動的設定檔**：聯賽開關、關注球隊、提醒分鐘數、補送視窗、摘要時間 |
...
| `digest.yml` | 每天 06:30 之後的第一次（見下） | 推播「接下來 24 小時有哪些比賽」 |
...
`digest.yml` 掛了 UTC 21 / 22 / 23 點（台灣 05:00 / 06:00 / 07:00），但**一天只會送一次**。

GitHub 的排程會遲到，實測這個 workflow 遲了 **103～118 分鐘**。如果只掛一個時間點、
把它往前推去抵銷遲到，就等於在賭「遲到幅度不會變」—— 哪天 GitHub 準時跑，摘要就會在
凌晨送到。

所以改成程式自己判斷：

- 現在還早於 `config.DIGEST_NOT_BEFORE`（預設 `06:30`）→ **這次不送**，交給下一槍。
- 今天已經送過（記在 `state.json` 的 `digest` 桶，key 是台灣日期）→ **不再送**。

結果：

| GitHub 遲到 | 哪一槍送出 | 送達時間 |
| --- | --- | --- |
| 0 分鐘（準時） | 23:00 那槍 | 07:00 |
| ~60 分鐘 | 22:00 那槍 | 07:00 |
| ~110 分鐘（實測值） | 21:00 那槍 | 06:50 |
| ~2 小時 | 21:00 那槍 | 07:00 |

不論遲到 0 到 2 小時，摘要都落在 06:30–08:00 之間且只送一次。想改時間就動
`config.py` 的 `DIGEST_NOT_BEFORE`；想手動測試用 `python main.py digest --force`
繞過這兩道限制。
...
- **偶發失敗**（對方 5xx、網路抖動）→ 只在 run 頁面標一個 `::warning::`，不紅燈。
  排程每 10 分鐘跑一次，偶發失敗天天寄信只會讓你之後把通知全部忽略。
- **持續失敗** → 某個聯賽連續失敗達 `config.FAIL_STREAK_ALERT` 次（預設 3 次，
  約 30 分鐘）就讓 workflow 紅燈，GitHub 會寄信給你。連續次數記在 `state.json`
  的 `failures` 桶，抓成功就歸零。

為什麼要這樣分：舊版只在**所有**聯賽都失敗時才紅燈，結果英超與西甲因為 ESPN
不吃日期區間而從頭到尾抓不到，卻因為只算「部分失敗」而永遠只印 warning ——
沒人會去翻每 10 分鐘一次的 run 頁面，兩個聯賽就這樣悄悄掛了好幾天。
...
python main.py digest       # 推播接下來 24 小時的賽程摘要（一天只送一次）
python main.py digest --force  # 忽略時間與去重限制，強制送出（測試用）
...
- **ESPN 是非官方公開端點**，沒有正式文件也沒有 SLA，隨時可能改結構或擋存取。程式已針對這點做了防護：單場解析失敗會跳過而非整批失敗、單一聯賽抓失敗不影響其他聯賽、User-Agent 被 WAF 擋下時會自動換一組重試、某個聯賽連續失敗就讓 workflow 紅燈寄信（見上面「聯賽壞掉了怎麼知道」）。ESPN 端點也已經不接受 `dates=起-迄` 的區間查詢，所以賽程是逐月（`dates=YYYYMM`）抓回來再自己切出需要的視窗。
