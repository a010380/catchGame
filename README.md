...
GitHub Actions 的 cron **不保證準時** —— 尖峰時可能延遲 5～15 分鐘。所以做法不是
「在開賽前 15 分鐘觸發一次」，而是**每 10 分鐘檢查一次**，把「還沒開打、且距離開賽
在 20 分鐘內」的比賽推出去，並記錄已推播過的避免重複。

結果是：**每場比賽一定會被提醒到，但實際送達時間落在開賽前約 20 到 5 分鐘之間。**
如果要更精準，只能改用有常駐主機的方案（VPS + crontab），那就得付月租。
...
- **開賽提醒的時間有 5～20 分鐘的浮動**，原因見上面「關於開賽提醒的準時度」。
- **賽果推播最慢會延遲 10 分鐘**（排程間隔）。
